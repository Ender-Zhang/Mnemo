    ...
    def test_web_client_asset_uses_polished_single_chat_shell(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn("<h1>Mnemo</h1>", html)
                self.assertIn('class="chat-subtitle"', html)
                self.assertIn("一个聊天框，完成所有事", html)
                self.assertIn("直接派活，Mnemo 会展示动作并记住偏好", html)
                ...
                self.assertIn('id="memoryMarkdownPanel"', html)
                ...
                self.assertIn("回到对话", html)
                self.assertNotIn("data-close-sheet", html)
                self.assertNotIn('class="sheet-backdrop"', html)
                self.assertIn('id="attachButton"', html)
                self.assertIn('id="mentionButton"', html)
                self.assertIn('id="toolIntentButton"', html)
                ...
                self.assertIn('target === "settings"', script)
                self.assertIn('view.classList.toggle("active", viewName === target)', script)
                self.assertNotIn("mobileSheetQuery", script)
                self.assertNotIn("isMobileSheetTarget", script)
                self.assertNotIn("isDesktopDrawerTarget", script)
                self.assertNotIn("mobile-sheet-open", script)
                self.assertNotIn("drawer-open", script)
                self.assertNotIn("drawer-active", script)
                self.assertNotIn("sheet-active", script)
                self.assertIn("loadMemoryCompass", script)
                self.assertIn("renderMemoryCompass", script)
                self.assertIn("renderMemoryCoverage", script)
                ...
                self.assertIn(".nav-bottom .quiet-button::before", css)
                ...
                self.assertNotIn("sheet-close", css)
                self.assertNotIn("sheet-backdrop", css)
                self.assertNotIn("body.drawer-open .sheet-backdrop", css)
                self.assertNotIn("drawer-active", css)
                self.assertNotIn("@keyframes drawer-slide", css)
                self.assertNotIn("sheet-active", css)
                self.assertNotIn("height: min(82dvh, 780px)", css)
                self.assertNotIn("@keyframes sheet-rise", css)
    ...
    def test_web_client_asset_renders_markdown_and_context_history(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn("isMarkdownDocumentStart", script)
                self.assertIn("isMarkdownHorizontalRule", script)
                self.assertIn("isMarkdownBlockquoteStart", script)
                self.assertIn("markdownBlockquote", script)
                ...
                self.assertIn('document.createElement("hr")', script)
                self.assertIn('document.createElement("blockquote")', script)
                self.assertIn('avatar.textContent = role === "user" ? "我" : "M"', script)
                ...
                self.assertIn(".markdown blockquote", css)
                self.assertIn(".markdown hr", css)
    ...
    def test_web_client_asset_deduplicates_activity_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn('event.data?.internal === "learning"', script)
    ...
    def test_web_client_asset_resolves_learning_chips_inline(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn("仅本次", script)
                self.assertIn("撤销", script)
                self.assertIn("记住", script)
    ...
    def test_web_client_asset_renders_settings_and_memory_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn('class="memory-index"', html)
                self.assertIn('class="memory-browser"', html)
                self.assertIn('class="memory-browser-grid"', html)
                self.assertIn('id="memoryCoverageLabel"', html)
                self.assertIn('id="memoryCoverageFill"', html)
                self.assertIn('id="memoryActiveTitle"', html)
                self.assertIn('id="memoryActiveSummary"', html)
                self.assertIn('id="memoryActiveMeta"', html)
                self.assertIn("记忆总览", html)
                self.assertIn("维度", html)
                self.assertIn('id="memoryMarkdownPanel"', html)
                self.assertIn('id="memorySelectedMeta"', html)
                self.assertIn("Memory Wiki", html)
                self.assertIn("本页查看", html)
                self.assertIn("选择记忆", html)
                self.assertIn("memory-empty-detail", html)
                self.assertNotIn("data-close-sheet", html)
                self.assertNotIn("isDesktopDrawerTarget", script)
                self.assertNotIn("drawer-active", script)
                self.assertIn('view.classList.toggle("active", viewName === target)', script)
                ...
                self.assertIn("renderMemoryCompass", script)
                self.assertIn("renderMemoryCoverage", script)
                self.assertIn("memoryCoverageFill.style.setProperty", script)
                self.assertIn("memoryActiveTitle", script)
                self.assertIn("memoryActiveSummary", script)
                self.assertIn("memoryActiveMeta", script)
                self.assertIn("memoryDimensionCard", script)
                self.assertIn("memory-dimension-head", script)
                self.assertIn("memory-dimension-counts", script)
                ...
                self.assertIn("openMemoryMarkdown", script)
                self.assertIn("hideMemoryMarkdown", script)
                ...
                self.assertIn("memory-browser-grid", css)
                self.assertIn("memory-coverage-track", css)
                self.assertIn("memory-compass", css)
                self.assertIn("memory-dimension-head", css)
                self.assertIn("memory-dimension-card", css)
                self.assertIn("memory-detail-card", css)
                self.assertIn("memory-selected-card", css)
                self.assertIn("memory-empty-detail", css)
                self.assertIn("memory-item-actions", css)
                self.assertIn("danger-button", css)
                self.assertIn("memory-markdown-panel", css)
                self.assertIn("memory-detail-evidence", css)
                self.assertNotIn(".drawer-active .memory-layout", css)
                self.assertNotIn(".drawer-active .settings-layout", css)
                self.assertNotIn(".drawer-active .provider-tiles", css)
    ...
    def test_web_client_asset_supports_enter_pending_and_tool_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn("insertComposerText", script)
