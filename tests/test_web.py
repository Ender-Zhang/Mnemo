    ...
    def test_web_client_asset_uses_polished_single_chat_shell(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertNotIn(".memory-view.drawer-active", css)
                self.assertIn(".settings-view.drawer-active", css)
                self.assertIn("@keyframes drawer-slide", css)
                self.assertNotIn("memory-view.sheet-active", css)
                self.assertIn("settings-view.sheet-active", css)
    ...
    def test_web_client_asset_renders_settings_and_memory_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with RunningServer(WebServerConfig(state_dir=tmp, port=0)) as server:
                ...
                self.assertIn("记忆罗盘", html)
                self.assertIn('class="memory-sidebar"', html)
                self.assertIn('class="memory-overview-card"', html)
                ...
                self.assertIn("settings-card", css)
                self.assertIn("memory-sidebar", css)
                self.assertIn("memory-overview-card", css)
                self.assertIn("memory-coverage-track", css)
                self.assertIn("memory-focus-heading", css)
                ...
                self.assertNotIn(".sheet-active .memory-panel", css)
                self.assertIn("memory-markdown-panel", css)
                self.assertIn("memory-detail-evidence", css)
                self.assertIn("memory-avatar-canvas", css)
                self.assertIn("memory-orbit", css)
                self.assertIn("width: min(100%, 260px)", css)
