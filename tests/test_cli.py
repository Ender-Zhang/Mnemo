...
class CliTests(unittest.TestCase):
    ...
    def test_channels_feishu_help_and_validation(self) -> None:
        ...
        self.assertIn("--no-streaming", help_result.stdout)
        self.assertIn("--footer-elapsed", help_result.stdout)
        self.assertIn("--thread-session", help_result.stdout)
    ...
    def test_service_dry_run_adds_feishu_sidecar_for_websocket_config(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('"--streaming"', launcher)
            self.assertIn('"--footer-status"', launcher)
            self.assertIn('"--footer-elapsed"', launcher)
            self.assertIn('"--thread-session"', launcher)
    ...
    def test_events_chat_and_replay_commands(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(replay_payload["passed"])
            self.assertEqual(replay_payload["mode"], "deterministic")
            self.assertGreater(replay_payload["event_count"], 0)

            replay_dry_run = _run_cli(["replay", run_id, "--state-dir", tmp, "--mode", "dry-run", "--json"])
            self.assertEqual(replay_dry_run.returncode, 0, replay_dry_run.stderr)
            dry_run_payload = json.loads(replay_dry_run.stdout)
            self.assertEqual(dry_run_payload["mode"], "dry_run")
            self.assertFalse(dry_run_payload["dry_run"]["model_called"])
            ...
            bad_replay_mode = _run_cli(["replay", run_id, "--state-dir", tmp, "--mode", "missing", "--json"])
            ...
            self.assertEqual(bad_replay_mode.returncode, 1)
            self.assertIn("mnemo: unknown replay mode", bad_replay_mode.stderr)
            self.assertNotIn("Traceback", bad_replay_mode.stderr)
    ...
    def test_harness_eval_and_replay_commands(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(replay_payload["passed"])
            self.assertGreater(replay_payload["event_count"], 0)
            replay_live = _run_cli(["harness", "replay", run_id, "--state-dir", tmp, "--mode", "live-tools", "--json"])
            self.assertEqual(replay_live.returncode, 0, replay_live.stderr)
            replay_live_payload = json.loads(replay_live.stdout)
            self.assertEqual(replay_live_payload["mode"], "live_tools")
            self.assertEqual(replay_live_payload["live_tools"]["summary"]["skipped"], 1)
    ...
    def test_api_schema_command_exposes_core_contract(self) -> None:
        ...
        self.assertIn("target", payload["methods"]["schedule_watch"]["input_schema"]["properties"])
        self.assertIn("message", payload["methods"]["schedule_cron"]["input_schema"]["properties"])
        ...
        self.assertIn("- schedule_watch:", text_result.stdout)
        self.assertIn("- schedule_cron:", text_result.stdout)
